 ...
 */
public abstract class RoutesMethods {

    /**
     * @param points points
     * @return distance of route in metres
     */
    public static double getDistance(ArrayList<Point> points) {
        double lat1;
        double lat2;
        double lon1;
        double lon2;
     ...
     */
    public static double[] getElevationGainLoss(ArrayList<Point> points) {
     ...
     */
    public static double[] getAltitudeMaxMin(ArrayList<Point> points) {
    ...
    public static double getMaxSpeed(ArrayList<Point> points) {
        ...
        if (maxSpeed <= 0) {
            ...
            for (int i = 0; i < points.size(); i++) {
                if (i != 0) {
                    ...
                    double speed = getSpeed(twoPoints);
                    if (speed > maxSpeed) {
                        maxSpeed = speed;
                    }
                    twoPoints.remove(0);
    ...
    public static double getSpeed(ArrayList<Point> points) {
        double timePast = points.get(0).getTime();
        double time = points.get(1).getTime();
        double timeDifference = (time - timePast) / 1000.0;
        return timeDifference == 0 ? 0 : getDistance(points) / timeDifference;
    }
     ...
     */
    public static double[] getHours(ArrayList<Point> points, boolean autoPause) {
        ...
        for (int i = 0; i < points.size(); i++) {
            if (i != 0) {
                ...
                double speed = getDistance(twoPoints) / (timeDifference / 1000.0);
     ...
     */
    public static ArrayList<LatLng> getLatLng(ArrayList<Point> points) {
     ...
     */
    private static double haversineFormula(double lat1, double lat2, double lon1, double lon2) {
        double r = 6371000;
     ...
     */
    public static String getDate(double time, String pattern) {
        DateFormat dateFormat = new SimpleDateFormat(pattern, Locale.getDefault());
        Date date = new Date((long) time);
        return dateFormat.format(date);
    }

    public static String[] getPace(double speed) {
    ...
    public static int[] getHoursMinutesSeconds(double time) {
        int hours = (int) time;
    ...
    public static String[] getMinutesSeconds(int minutes, int seconds) {
